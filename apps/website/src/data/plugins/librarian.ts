...
export const librarian: PluginData = {
  ...
  hero: {
    headline: 'The same decision, rediscovered every session. Librarian ends it.',
    subheadline:
  ...
  tables: [
    {
      ...
      rows: [
        ...
        ['feedback', 'Corrections or validated preferences ("don\'t do X", "yes, keep doing Y") — includes Why and How to apply lines'],
        ['project', 'Ongoing work facts, decisions, constraints not derivable from the code — includes Why and How to apply lines'],
        ['reference', 'Pointers to external systems (issue trackers, dashboards, channels)'],
        ['null', 'Session-only — interesting but not durable; classifier opted out of promotion'],
      ],
    },
    {
      ...
      rows: [
        ...
        ['duplicate', 'Very high similarity to an existing memory — dropped silently as redundant'],
        ['merge_candidate', 'Partial overlap — surfacer offers "merge into existing memory X"'],
        ['conflict_candidate', 'Opposing sentiment markers vs. existing memory — surfacer offers "supersede / keep both / drop new"'],
      ],
    },
    {
      title: 'Status',
      ...
      rows: [
        ['Hook entry points + scaffolding', 'Ships now — load cleanly, archivist artifact paths wired'],
        ['Scan + classify + propose pipeline', 'In design / scaffolding phase — not yet implemented'],
        ['/librarian review interactive walkthrough', 'Deferred — accept / reject / edit / merge / supersede per proposal'],
        ['Auto-promote at high confidence', 'Off by default — ADR-001 commits to propose-don\'t-auto-write'],
      ],
    },
  ],
  ...
  events: [
    ...
    },
    {
      ...
      keyFields: 'proposal_id, proposed_type, conflict_state, classifier_confidence',
    },
  ],
