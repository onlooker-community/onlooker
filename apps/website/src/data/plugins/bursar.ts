	...
	tables: [
		{
			...
			rows: [
				[
					"Is this single session over budget?",
					"Governor — per-session cap, enforced live",
				],
				[
					"What has this project cost me this week?",
					"Bursar — per-project rollup, surfaced at SessionStart",
				],
				["Was this turn unusually expensive?", "Governor — per-turn deltas"],
				[
					"Should I notice that I started thirteen sessions on this repo since Monday?",
					"Bursar — windowed session count + cost",
				],
			],
		},
		{
			...
			rows: [
				[
					"rolling_7d (default)",
					"Sums the trailing 7×24h from the moment of SessionStart",
				],
				[
					"calendar_week",
					"Sums from the most recent week start; honors week_start (monday or sunday)",
				],
			],
		},
	],
	...
	events: [
		{
			...
			keyFields:
				"project_key, session_id, governor_present, cost_usd?, tokens?, api_calls?, model?",
		},
		{
			type: "bursar.rollup.surfaced",
			when: "At SessionStart, when a windowed total is shown",
			keyFields:
				"project_key, window, window_start, total_cost_usd, session_count, total_tokens, sessions_with_cost",
		},
		{
