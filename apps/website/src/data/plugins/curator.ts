  ...
  tables: [
    {
      ...
      rows: [
        ['date_decayed', 'ISO-8601 date in a memory body that has passed the grace period and may no longer apply'],
        ['path_broken', 'Path reference in a memory body that no longer resolves against the repo root'],
        ['broken_index', 'MEMORY.md links a file that does not exist on disk'],
        ['orphaned_memory', 'A *.md file in the memory dir that no MEMORY.md entry points to'],
      ],
    },
    {
      ...
      rows: [
        ['LLM contradiction sweep', 'Watermark plumbing is in place; the Haiku pair-evaluation loop is not implemented yet'],
        ['Usage tracker', 'Depends on a substrate-level memory.recalled emitter that does not exist yet'],
        ['Symbol reference check', 'Backtick-wrapped identifiers grep\'d against the repo — not yet wired'],
        ['/curator review walkthrough', 'Accept / prune / edit / reclassify / acknowledge / defer for surfaced findings'],
      ],
    },
  ],
