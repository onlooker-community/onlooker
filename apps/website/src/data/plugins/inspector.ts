...
export const inspector: PluginData = {
  ...
  hero: {
    headline: 'After every Write, Edit, MultiEdit — does the code actually compile?',
    subheadline:
  ...
  tables: [
    {
      ...
      rows: [
        ['Inspector', 'PostToolUse on Edit / Write / MultiEdit — per touched file, cheap, fires often'],
        ['Proctor (planned)', 'Stop — full project verification command'],
        ['Assayer', 'Stop — checks the agent\'s claims against transcript evidence; needs Inspector\'s real signals to work against'],
        ['Build system', 'Not Inspector\'s job — no cross-file caching, no dependency graphs'],
      ],
    },
    {
      ...
      rows: [
        ...
        ['total_budget_exhausted', 'Run exceeded total_timeout_seconds; remaining checks did not run'],
      ],
    },
  ],
  ...
  events: [
    {
      ...
      keyFields: 'file_path, tool_name, check_name, check_kind, argv, duration_ms',
    },
    {
    ...
    },
    {
      ...
      keyFields: 'reason (disabled | excluded_path | no_extension_match | not_in_repo | tool_missing | timeout | total_budget_exhausted)',
    },
    {
      type: 'inspector.run.completed',
      when: 'Once per hook fire after all checks',
      keyFields: 'checks_run, checks_passed, checks_failed, checks_skipped, duration_ms',
    },
  ],
