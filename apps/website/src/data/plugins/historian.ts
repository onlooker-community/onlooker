	...
	tables: [
		{
			...
			rows: [
				[
					"SessionEnd indexing pipeline",
					"Ships now — transcript reader, chunker, sanitizer, JSONL store",
				],
				[
					"Append-only JSONL chunk records",
					"Ships now — bodies stored without vectors; adding embeddings later is a column-add",
				],
				[
					"Secret redaction + [historian:skip] markers + path-deny list",
					"Ships now",
				],
				[
					"Retrieval and additionalContext surfacer",
					"Deferred — UserPromptSubmit hook currently no-ops",
				],
				[
					"Embedder backends (ollama, fastembed, remote)",
					"Deferred — chunks indexed without vectors until the first backend lands",
				],
				[
					"/historian recall, setup, stats, purge",
					"Deferred — slash commands ship with retrieval",
				],
		...
		},
		{
			...
			rows: [
				[
					'"We\'ve solved this exact bug before"',
					"The past session's investigation chunks — root cause, fix, and rejected hypotheses",
				],
				[
					'"I tried X already and it didn\'t work"',
					"The dead-end discussion, not just a feedback-store conclusion — the why behind the rejection",
				],
				[
					'"What was the rationale we settled on?"',
					"The 40-turn tradeoff weighing that became a one-line commit message",
				],
			],
		},
	],
	...
	events: [
		{
			...
			keyFields:
				"session_id, chunks_written, redaction_count, body_chars_total",
		},
		{
