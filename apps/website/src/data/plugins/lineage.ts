	...
	tables: [
		{
			...
			rows: [
				[
					"/lineage <file>",
					"Full change history for the file, newest first, each with its resolved prompt context",
				],
				[
					"/lineage <file>:<line>",
					"Which change introduced the content currently on line N — with the prompt / agent / session behind it",
				],
				[
					"/lineage <file> --grep <text>",
					"Which change introduced content matching <text>",
				],
				[
					"/lineage --status",
					"Ledger stats for the project (changes recorded, files touched)",
				],
			],
		},
		{
			...
			rows: [
				[
					"historian_then_transcript (default)",
					'Prefer Historian (durable JSONL chunks); fall back to the live transcript; then to "prompt unavailable"',
				],
				[
					"historian_only",
					"Only resolve via Historian; never read the live transcript",
				],
				[
					"transcript_only",
					"Only resolve via the live transcript; ignore Historian",
				],
			],
		},
	],
	...
	events: [
		{
			...
			keyFields:
				"project_key, session_id, file_path, tool, operation, change_id, lines_added, lines_removed, bytes, edit_count, content_sha256",
		},
		{
