...
for app in api web; do
	...
	strays="$(grep -rlE "${PROVIDER_IMPORT}" "${app_src}" 2>/dev/null |
		grep -v "/src/monitoring\.ts$" || true)"

	if [[ -n "${strays}" ]]; then
		fail "apps/${app} reaches the provider only through src/monitoring.ts" \
			"also imported by: ${strays//${ROOT}\//}"
	else
		pass "apps/${app} reaches the provider only through src/monitoring.ts"
	fi
	...
	if [[ -f "${app_src}/monitoring.ts" ]]; then
		if grep -qE "${PROVIDER_IMPORT}" "${app_src}/monitoring.ts"; then
			pass "apps/${app}/src/monitoring.ts is what the provider pattern matches"
		else
			fail "apps/${app}/src/monitoring.ts is what the provider pattern matches" \
