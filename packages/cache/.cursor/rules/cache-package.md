...
- **Result Types**: Core operations return `Result<T, CacheError>` for consistent and explicit error handling
- **Never-Failing Wrappers**: Wrappers like `withCache()` always return function result, handling cache errors internally without throwing
...
- **Validates** `ol:resource:identifier[:subresource]*` pattern
...
- **Logger Integration**: Uses `@onlooker/logger` with structured logging
...
const globalForCache = globalThis as unknown as {
  ...
  onlookerCacheInitializing: Promise<Result<CacheService, CacheError>> | undefined;
};
