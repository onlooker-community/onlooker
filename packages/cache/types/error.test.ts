...
describe("Error types and utilities", () => {
	...
	describe("err utility function", () => {
		...
		test("should work with different error types", () => {
			...
			if (!cacheResult.ok)
				expect(cacheResult.error.code).toBe(ErrorCode.CacheValidationError);
			if (!redisResult.ok)
				expect(redisResult.error.code).toBe(ErrorCode.RedisOperationError);
		});
	});
	...
	describe("ErrorCode enum", () => {
		...
		test("should be usable as object keys", () => {
			...
			expect(errorMap[ErrorCode.CacheValidationError]).toBe(
				"Validation failed",
			);
		});
	});
	...
	describe("Result type", () => {
		test("should discriminate between success and error states", () => {
			...
			const errorResult: Result<string, CacheError> = err({
				code: ErrorCode.Unknown,
			});
		...
		test("should support type safety with different data types", () => {
			...
			const objectResult: Result<{ id: string }, CacheError> = ok({
				id: "123",
			});
	...
	describe("CacheErrorClass", () => {
		test("should create proper Error instances with code and message", () => {
			const error = new CacheErrorClass(
		...
		describe("fromCacheError static method", () => {
			...
			test("should use custom message when provided", () => {
				...
				const errorClass = CacheErrorClass.fromCacheError(
					plainError,
		...
		test("should maintain proper prototype chain", () => {
			...
			expect(Object.getPrototypeOf(CacheErrorClass.prototype)).toBe(
				Error.prototype,
			);
