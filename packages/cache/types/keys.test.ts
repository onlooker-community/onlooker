...
describe("@onlooker/cache types/keys", () => {
	...
	describe("CacheKey type", () => {
		...
		test("should be serializable", () => {
			...
			expect(JSON.stringify({ cacheKey: key })).toBe(
				'{"cacheKey":"ol:serialization:test"}',
			);

			// Should parse back correctly
			const parsed = JSON.parse('{"cacheKey":"ol:serialization:test"}') as {
				cacheKey: string;
			};
	...
	describe("CustomCacheNamespace type", () => {
		test("should support known custom namespaces in parsed cache keys", () => {
			...
			const cacheKeys = namespaces.map((namespace) =>
				ZCacheKey.parse(`${namespace}:test:123`),
			);
