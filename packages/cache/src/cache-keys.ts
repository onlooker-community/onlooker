import type { CacheKey, CustomCacheNamespace } from "@/types/keys";
import { makeCacheKey } from "./utils/key";

/**
 * Enterprise-grade cache key generator following industry best practices
 * Pattern: ol:\{resource\}:\{identifier\}:\{subResource\}
 *
 * Benefits:
 * - Clear namespace hierarchy (ol = onlooker)
...
export const createCacheKey = {
	...
	workspace: {
		state: (workspaceId: string): CacheKey =>
			makeCacheKey("env", workspaceId, "state"),
		config: (workspaceId: string): CacheKey =>
			makeCacheKey("env", workspaceId, "config"),
		segments: (workspaceId: string): CacheKey =>
			makeCacheKey("env", workspaceId, "segments"),
	},

	// Organization-related keys
	organization: {
		billing: (organizationId: string): CacheKey =>
			makeCacheKey("org", organizationId, "billing"),
	},

	// License and enterprise features
	license: {
		status: (organizationId: string): CacheKey =>
			makeCacheKey("license", organizationId, "status"),
		previous_result: (organizationId: string): CacheKey =>
			makeCacheKey("license", organizationId, "previous_result"),
		fetch_lock: (organizationId: string): CacheKey =>
			makeCacheKey("license", organizationId, "fetch_lock"),
	},

	// Response-related keys
	response: {
		countBySurveyId: (surveyId: string): CacheKey =>
			makeCacheKey("response", surveyId, "count"),
	},
	...
	custom: (
		namespace: CustomCacheNamespace,
		identifier: string,
		subResource?: string,
	): CacheKey => {
