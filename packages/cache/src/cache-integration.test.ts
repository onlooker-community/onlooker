...
describe("Cache Integration Tests - End-to-End Redis Operations", () => {
	beforeAll(async () => {
		...
		if (!isRedisAvailable) {
			logger.info(
				"🟡 Cache Integration Tests: Redis not available - tests will be skipped",
			);
			logger.info(
				"   To run these tests locally, ensure Redis is running and REDIS_URL is set",
			);
			return;
		}
		...
		if (cacheService) {
			...
			if (redis) {
				const testKeys = await redis.keys("ol:cache:test:*");
	...
	test("withCache miss/hit pattern: first call miss, second call hit", async () => {
		...
		const expensiveFunction = async (): Promise<{
			result: string;
			timestamp: number;
			execution: number;
		}> => {
		...
		const firstCall = await cacheService.withCache(
			expensiveFunction,
			key,
			60000,
		);
		...
		logger.info(
			`✅ First call executed function: execution=${firstCall.execution}`,
		);

		logger.info("Second call (cache hit expected)...");
		const secondCall = await cacheService.withCache(
			expensiveFunction,
			key,
			60000,
		);
		...
		logger.info(
			`✅ Second call returned cached value: execution=${secondCall.execution}`,
		);
	...
	test("Cache invalidation: del() clears cache and forces recomputation", async () => {
		...
		const firstResult = await cacheService.withCache(
			expensiveFunction,
			key,
			60000,
		);
		...
		const secondResult = await cacheService.withCache(
			expensiveFunction,
			key,
			60000,
		);
		...
		logger.info(
			"Third call after invalidation - should miss cache and recompute...",
		);
		const thirdResult = await cacheService.withCache(
			expensiveFunction,
			key,
			60000,
		);
		...
		logger.info(
			`✅ Cache miss after invalidation confirmed: ${thirdResult.value}`,
		);

		logger.info("Fourth call - should hit cache again...");
		const fourthResult = await cacheService.withCache(
			expensiveFunction,
			key,
			60000,
		);
	...
	test("TTL expiry behavior: cache expires automatically", async () => {
		...
		logger.info(
			"Third call after TTL expiry - should miss cache and recompute...",
		);
		const thirdResult = await cacheService.withCache(
			expensiveFunction,
			key,
		...
		expect(thirdResult).not.toEqual(firstResult);
		logger.info(
			`✅ Cache miss after TTL expiry confirmed: ${thirdResult.value}`,
		);
		...
		if (redis) {
			...
			const currentKeys = await redis.keys(`ol:cache:${key}*`);
			logger.info(
				`Current cache keys: ${currentKeys.length > 0 ? currentKeys.join(", ") : "none"}`,
			);
	...
	test("Concurrent cache operations: thread safety", async () => {
		...
		const redis = cacheService.getRedisClient();
		if (redis) {
			const existingKeys = await redis.keys(`ol:cache:${baseKey}*`);
		...
		results.forEach(({ i, firstCall, secondCall }) => {
			...
			logger.info(
				`Operation ${i}: first=${firstCall.execution}, second=${secondCall.execution} (cached)`,
			);
		});
	...
	test("Different data types: serialization correctness", async () => {
		...
		for (const testCase of testCases) {
			const key = createCacheKey.workspace.state(
				`serialization-${testCase.name}`,
			);
		...
		logger.info(
			"✅ withCacheNullable correctly ignores entries written via raw cache.set(null)",
		);
	...
	test("Error handling: graceful degradation when operations fail", async () => {
		...
		if (!setResult.ok) {
			...
			logger.info(
				`✅ Set with invalid TTL handled gracefully: ${setResult.error.code}`,
			);
		}
		...
		logger.info(
			"✅ withCache gracefully degraded to function execution when cache failed",
		);
	...
	test("withCacheNullable: round-trips real values and cached nulls without re-executing fn", async () => {
		...
		const realFirst = await cacheService.withCacheNullable(
			realFn,
			realKey,
			60000,
		);
		const nullFirst = await cacheService.withCacheNullable(
			nullFn,
			nullKey,
			60000,
		);
		...
		const realSecond = await cacheService.withCacheNullable(
			realFn,
			realKey,
			60000,
		);
		const nullSecond = await cacheService.withCacheNullable(
			nullFn,
			nullKey,
			60000,
		);
		...
		await cacheService.del([realKey, nullKey]);
		logger.info(
			"✅ withCacheNullable round-trip confirmed for real and null values",
		);
	}, 15000);
});
