...
const NULLABLE_BOX_MARKER = "__ol_nullable_v1";
...
export class CacheService {
		...
		try {
			...
			if (ttlMs === undefined) {
				...
				await this.withTimeout(
					this.redis.setEx(key, Math.floor(ttlMs / 1000), serialized),
				);
			}
			return ok(undefined);
		} catch (error) {
	...
	private canUseCache(key: CacheKey, ttlMs: number): boolean {
		...
		if (!validation.ok) {
			logger.warn(
				{ error: validation.error, key },
				"Invalid cache inputs, executing function directly",
			);
			return false;
		}
	...
	private async tryGetCachedValue<T>(
		key: CacheKey,
		ttlMs: number,
	): Promise<T | undefined> {
		try {
			...
			logger.debug(
				{ error, key, ttlMs },
				"Cache get threw; proceeding to compute fresh value",
			);
		}
	...
	private async trySetCache(
		...
		if (value === undefined || value === null) {
			logger.debug(
				{ key, ttlMs },
				"Refusing to cache nullish value; treating as non-cacheable",
			);
			return;
		}
		...
		try {
			...
			logger.debug(
				{ error, key, ttlMs },
				"Cache set threw; returning fresh result",
			);
		}
	}
