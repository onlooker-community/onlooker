...
describe("@onlooker/cache utils/key", () => {
	describe("makeCacheKey helper", () => {
		...
		test("should work with many parts", () => {
			const key: CacheKey = makeCacheKey(
				"user",
				"123",
				"org",
				"456",
		...
		test("should throw error for empty parts", () => {
			expect(() => makeCacheKey("env", "", "state")).toThrow(
				"Invalid Cache key: Parts cannot be empty",
			);

			expect(() => makeCacheKey("", "123")).toThrow(
				"Invalid Cache key: Parts cannot be empty",
			);
		});
		...
		test("should validate structure with regex", () => {
			...
			expect(() =>
				makeCacheKey("rate_limit", "api", "user", "123"),
			).not.toThrow();
		});
		...
		test("should be compatible with existing cache key patterns", () => {
			...
			expect(makeCacheKey("env", "env-123", "state")).toBe(
				"ol:env:env-123:state",
			);
			expect(makeCacheKey("org", "org-456", "billing")).toBe(
				"ol:org:org-456:billing",
			);
			expect(makeCacheKey("license", "org-789", "status")).toBe(
				"ol:license:org-789:status",
			);
