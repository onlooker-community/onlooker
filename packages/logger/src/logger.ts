...
const buildTransport = (): LoggerOptions["transport"] => {
	...
	const serviceName = process.env.OTEL_SERVICE_NAME ?? "onlooker";
	...
	const prettyTarget = {
		...
		options: {
			...
			customLevels:
				"trace:10,debug:20,info:30,warn:40,error:50,fatal:60,audit:90",
...
const boundLogger = {
	...
	audit: (pinoLogger as Logger & { audit: typeof pinoLogger.info }).audit.bind(
		pinoLogger,
	),
