/* eslint-disable no-console -- Console logs are allowed for plugins*/
...
export function copyCompiledAssetsPlugin(
	options: CopyCompiledAssetsPluginOptions,
): Plugin {
		...
		async writeBundle() {
			try {
				...
				try {
					...
					if ((error as { code: string }).code === "ENOENT") {
						console.error(
							`Error: Distribution directory ${distDir} does not exist`,
						);
						if (!options.skipDirectoryCheck) {
							throw error;
						} else {
							console.log(
								`Skipping directory check as skipDirectoryCheck is enabled`,
							);
				...
				console.log(
					`Copied ${String(copiedFiles)} files to ${outputDir} (${options.filename})`,
				);
			} catch (error) {
				if (options.skipDirectoryCheck) {
					console.error(
